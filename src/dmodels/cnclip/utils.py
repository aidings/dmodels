...
def create_model(model_name, checkpoint=None):
    conf_root = data_root / 'cnclip'
    vision_model, text_model = model_name.split('@')
    # Initialize the model.
    vision_model_config_file = conf_root / f"model_configs/{vision_model.replace('/', '-')}.json"
    print('Loading vision model config from', vision_model_config_file)
    assert os.path.exists(vision_model_config_file)

    text_model_config_file = conf_root / f"model_configs/{text_model.replace('/', '-')}.json"
